...
struct ScreenInfo {
    ...
    string scale_mul;      // Formatted scale multiplier (e.g., "1x", "1.75x")
    string upscale;        // GPU upscale info (e.g., "Off", "2x (NVIDIA DSR)", "4x (AMD VSR)")
};
...
public:
    ...
    vector<ScreenInfo> getScreens() const { return screens; }
